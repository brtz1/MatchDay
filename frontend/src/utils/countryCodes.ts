 ...
 */
export function toISO(countryOrCode: string): string {
  ...
  if (isLikelyCode(raw)) {
    ...
    if (code.startsWith("gb-")) return code;
...
export function getFlagUrl(countryOrCode: string): string {
  ...
  if (!fileCode) return ""; // caller can hide img on empty src
...
/* ---------------------------------------------------------------------- */
export const COUNTRY_TO_ISO: Record<string, string> = NAME_TO_ISO;
